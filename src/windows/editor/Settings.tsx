...
import { IconCheck, IconClose, IconFolder } from "@/components/icons";
import { IconButton } from "@/components/ui/IconButton";
import * as ipc from "@/lib/ipc";
import type { BackupSettings, BackupTarget } from "@/lib/types";

/**
 * Settings. Currently one thing: keeping a second copy of every capture.
 *
...
export function Settings({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="surface-pop animate-in-pop flex max-h-full w-[min(520px,95vw)] flex-col overflow-hidden rounded-2xl"
        role="dialog"
        aria-label="Settings"
      >
            ...
            <>
              <div className="space-y-1">
                ...
                {settings.destination &&
                  !targets.some((t) => t.path === settings.destination) && (
                    <Choice
                      label="Chosen folder"
              ...
              {note && (
                <p
                  className={clsx(
                    "mt-2.5 text-[11.5px]",
                    note.bad ? "text-danger" : "text-ink-3",
                  )}
                >
                  {note.text}
                </p>
