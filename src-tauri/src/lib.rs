...
fn build_menu(app: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    if let Some(MenuItemKind::Submenu(app_menu)) = menu.items()?.into_iter().next() {
        ...
        // The licences of everything Shotly is built on. MIT and its cousins
        // ask for the notice to travel with the binary; this is where it is.
        let notices = MenuItem::with_id(app, "notices", "Acknowledgements", true, None::<&str>)?;
        app_menu.insert(&website, 1)?;
        app_menu.insert(&notices, 2)?;
        app_menu.insert(&sep, 3)?;
        app_menu.insert(&settings, 4)?;
    }
    ...
    TrayIconBuilder::with_id("shotly-tray")
        ...
        .on_menu_event(|app, event| match event.id().as_ref() {
            ...
            "notices" => {
                ...
                use tauri_plugin_opener::OpenerExt;
                // Bundled as a resource by tauri.conf.json, regenerated by
                // `npm run notices` before every release.
                let opened = app
                    .path()
                ...
                if let Err(err) = opened {
                    eprintln!("[shotly] could not open the third-party notices: {err}");
                }
            }
            "quit" => app.exit(0),
            _ => {}
